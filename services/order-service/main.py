...
async def lifespan(app: FastAPI):
    ...
    # Declare the queue so it will exist in rabbitmq.
    # IMPORTANT: These arguments MUST match what inventory-service declares,
    # otherwise RabbitMQ rejects the declaration with a PRECONDITION_FAILED error.
    async with rabbitmq_connection.channel() as channel:
        arguments = {
            "x-dead-letter-exchange": "dlx",
            "x-dead-letter-routing-key": "order_events",
        }
        await channel.declare_queue("order_events", durable=True, arguments=arguments)
